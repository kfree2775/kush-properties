...
export async function render(el) {
  ...
  el.innerHTML = `<div class="admin-card"><form id="ck-form" class="admin-form">
    <div class="form-group"><label>Enabled</label>
      <div class="admin-toggle"><div class="admin-toggle__switch ${config.enabled!==false?'on':''}" id="ck-enabled"></div><span class="admin-toggle__label">${config.enabled!==false?'Yes':'No'}</span></div>
    </div>
    <div class="form-group"><label>Banner Message</label><textarea id="ck-msg" rows="3">${config.bannerText || ''}</textarea></div>
    <div class="form-row">
  ...
  el.querySelector('#ck-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      await apiFetch('/cookie', { method: 'PUT', body: JSON.stringify({
        enabled: el.querySelector('#ck-enabled').classList.contains('on'),
        bannerText: el.querySelector('#ck-msg').value, acceptText: el.querySelector('#ck-accept').value, declineText: el.querySelector('#ck-decline').value,
      })});
